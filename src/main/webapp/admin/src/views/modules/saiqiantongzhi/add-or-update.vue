...
export default {
  ...
  methods: {
	...
	this.ruleForm.tongzhineirong = this.ruleForm.tongzhineirong.replace(reg,'../../../ssm4o4c3/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `saiqiantongzhi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.saiqiantongzhiCrossAddOrUpdateFlag = false;
                  this.parent.search();
